...
---
Task ID: 2
Agent: full-stack-developer (decoded content renderers)
Task: Add decoded content renderers to the evidence detail panel

Work Log:
- Read worklog.md (Tasks 5, 6, 7, 8 history) to understand prior agent work, then read the full evidence-view.tsx file (1510 lines) to map the existing DetailPanel structure (Sheet header → friendly preview → Metadata section → Technical Details (advance-mode) → Confidence Assessment → Integrity/SHA-256 → Tags → Notes → Export → Raw JSON → Delete).
- Confirmed `ApiEvidenceItem.decodedContent: string | null` already exists in `src/lib/types.ts` (line 214) and is populated by `src/lib/scan-engine.ts` (JSON-stringified per-category payloads for photos/audio/sms/credentials/installed_apps). Confirmed `formatBytes`, `formatDateTime`, `cn`, `copyToClipboard`, `toast`, `MetaRow`, `Tooltip`, `Badge`, `Button`, `Separator`, `COLOR_CLASSES`, `IconType` already available in scope.
- Added 11 new lucide-react icon imports to the existing import block: Eye, EyeOff, Play, Pause, Camera, KeyRound, Phone, Volume2, ShieldAlert, Wifi, Clock.
- Implemented a `parseDecoded(item)` helper that safely JSON-parses `item.decodedContent` and returns `Record<string, unknown> | null` (try/catch + object/array guard).
- Added typed accessor helpers `asString`, `asNumber`, `asBool`, `asStringArray` plus a small `DecodedSectionHeader` presentational component (icon + uppercase label) reused by all renderers for visual consistency.
- Implemented 5 category-specific renderers + 1 generic fallback, all defined BEFORE DetailPanel so they're in scope:
  • `PhotoDecoded` — thumbnail `<img>` (data URL) preview, EXIF metadata table (dimensions, camera make+model, focal length, aperture, ISO, exposure), GPS card (lat/lon to 6 decimals, altitude, location name).
  • `AudioDecoded` — simulated audio player: round play/pause Button toggling `isPlaying` state, 40-bar waveform visualization where bars up to `progress` % are highlighted in primary color, live progress time + duration label, 200ms-tick `setInterval` driving `progress` 0→100 with auto-stop effect; Audio Metadata table (duration, codec, sample rate, channels Mono/Stereo, bitrate, language); Transcription card with italic quoted body and color-coded confidence badge (≥90 green / ≥75 amber / else red).
  • `SmsDecoded` — chat-bubble layout: incoming left-aligned (muted bg) / outgoing right-aligned (primary bg, items-end); sender/recipient name + phone above the bubble; body in readable English; Clock icon + formatted `item.createdAtDevice` timestamp inside bubble; metadata table (direction, sender, recipient, phone, message type, language) and color-coded read-status Badge (read=green / unread=red / delivered=blue).
  • `CredentialsDecoded` — amber warning banner ("Credentials extracted from device keychain — handle per chain-of-custody policy.") with ShieldAlert icon; service header card with KeyRound icon, service name, account, token-type badge; password row with Eye/EyeOff show/hide toggle (default `showPassword=false`, masked as •••) + Copy button (sonner toast); token value row with Copy button; metadata table (extraction method, accessible-when-unlocked, security-level Badge: strong=green Shield / moderate=amber Shield / weak=red ShieldAlert).
  • `InstalledAppsDecoded` — app header card (AppWindow icon, app name, category badge, bundle ID mono); metadata table (bundle ID, version, install date, last used, data size, cache size — sizes formatted via `formatBytes` with HardDrive icon); permissions chips color-coded via `PERMISSION_COLORS` map (camera/microphone=red, location=amber, contacts=blue, storage=gray, others=slate); status badges row: has-credentials (green KeyRound if true), network-activity (Wifi icon, color by high/moderate/low/none), sandboxed (green Shield if true, red Shield if false).
  • `GenericJsonDecoded` — fallback for contacts/browser_history/call_logs/emails/notes/etc.: iterates `Object.entries(data)`, humanizes camelCase/snake_case keys, formats values (string/number/boolean/array/object) via the existing `MetaRow` primitive with mono font for technical values; renders nothing if entries is empty.
- Implemented `DecodedContentRenderer` switcher component that calls `parseDecoded`, returns null if no decoded content, otherwise switches on `item.category` to delegate to the appropriate renderer (default → GenericJsonDecoded).
- Inserted `<DecodedContentRenderer item={item} />` between the Confidence Assessment section and the Integrity/SHA-256 section inside DetailPanel (per task spec: "after the metadata table and before the integrity/hash section").
- Fixed a pre-existing TypeScript error in the same file: `CATEGORY_META` (a `Record<EvidenceCategory, CategoryMeta>`) was missing the `credentials` and `installed_apps` keys, which caused `tsc --noEmit` to fail and would have crashed DetailPanel at runtime when those items were opened (since `const meta = CATEGORY_META[item.category]` would be undefined). Added two entries: `credentials: { label: "Credentials", icon: KeyRound, color: "amber" }` and `installed_apps: { label: "Installed Apps", icon: AppWindow, color: "violet" }`. This fix is required for my new CredentialsDecoded/InstalledAppsDecoded renderers to actually be reachable.
- Visual consistency: every renderer uses `rounded-md border border-border/60 bg-muted/20 p-3` containers, `font-mono-forensic` for technical values, `divide-y divide-border/40` for row separators, and `text-[11px] uppercase tracking-wider text-muted-foreground` for section headers — matching the existing DetailPanel aesthetic.
- Ran `bun run lint` — exit code 0, zero errors, zero warnings in evidence-view.tsx (and project-wide).
- Ran `npx tsc --noEmit --skipLibCheck` filtered to evidence-view.tsx — zero TypeScript errors after the CATEGORY_META fix.
- Verified dev server (dev.log tail): "✓ Compiled in 173ms" after edits, all routes returning 200, no runtime errors.

Stage Summary:
- Single file modified: `/home/z/my-project/src/components/views/evidence-view.tsx` (added ~810 lines of new renderer code + 2-line CATEGORY_META fix; no other files touched, no new files created).
- Added `parseDecoded` helper, 5 category-specific renderers (PhotoDecoded, AudioDecoded, SmsDecoded, CredentialsDecoded, InstalledAppsDecoded), 1 generic fallback (GenericJsonDecoded), and a `DecodedContentRenderer` switcher.
- Inserted the renderer into the DetailPanel Sheet between Confidence Assessment and Integrity/SHA-256 sections — renders nothing when `decodedContent` is null/invalid, so non-decoded categories are unaffected.
- Interactive features: audio play/pause with simulated progress + waveform highlight, password show/hide toggle (default hidden), copy-to-clipboard for password and token value with sonner toasts, tooltips on icon buttons, accessible ARIA labels.
- Fixed pre-existing `CATEGORY_META` TypeScript error by adding `credentials` and `installed_apps` entries (required for the new renderers to be reachable at runtime).
- Lint clean, TypeScript clean, dev server healthy. No new API routes or schema changes.
